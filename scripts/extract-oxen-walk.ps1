param(
  [string]$InputPath = 'assets\protectors\oxen walk.png',
  [string]$OutputDirectory = 'assets\protectors\oxen-walk'
)
    ...
    for ($column = 0; $column -lt $columns; $column += 1) {
      ...
      try {
        ...
        try {
          ...
          $graphics.DrawImage($source, [Drawing.Rectangle]::new($destinationX, $destinationY, $cellWidth, $cellHeight), [Drawing.Rectangle]::new($left, $top, $cellWidth, $cellHeight), [Drawing.GraphicsUnit]::Pixel)
...
Write-Output "Extracted $frame oxen frames in row-major order to $outputPath ($canvasWidth x $canvasHeight each)."
